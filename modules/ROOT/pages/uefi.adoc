...
| `UEFI_070` | The implementation MUST declare the appropriate BRS UEFI conformance profile GUID as defined in the section <<uefi-conformance-guids>>.
...
[discrete]
[[uefi-conformance-guids]]
=== BRS-I UEFI Conformance GUIDs
The EFI Conformance Profile Table (UEFI specification, Section 4.6.4 cite:[UEFI]) defines the following GUIDs to indicate conformance to particular versions of the BRS specification.

[width=100%]
[%header, cols="5,25,15"]
|===
| BRS Version ^| GUID Name                                 | GUID
| 1.0          | EFI_CONFORMANCE_PROFILE_BRS_1_0_SPEC_GUID | `({ 0x05453310, 0x0545, 0x0545, { 0x05, 0x45, 0x33, 0x05, 0x45, 0x33, 0x05, 0x45 }})`
| 1.1          | EFI_CONFORMANCE_PROFILE_BRS_1_1_SPEC_GUID | `({ 0x63C65EB9, 0x27EE, 0x43A1, { 0xBD, 0xD1, 0x3C, 0x91, 0x03, 0xA4, 0x86, 0xCA }})`
3+| _Only a system fully compliant to the requirements in the section <<uefi>> MUST declare the appropriate conformance profile._
|===
...
| `USEC_060` | If the firmware allows forward-edge control-flow integrity (FCFI) to be enabled for
the supervisor execution environment, the runtime services MUST be compiled to support FCFI.
2+| The supervisor execution environment SHOULD enable FCFI through the SBI FWFT LANDING_PAD
interface.
| `USEC_070` | The support for forward-edge control-flow integrity in runtime services MUST be
signaled by the EFI_MEMORY_ATTRIBUTES_FLAGS_RT_FORWARD_CONTROL_FLOW_GUARD flag cite:[UEFI].
| `USEC_080` | If the runtime services support forward-edge control-flow integrity, the instruction
at the entry address of any runtime service MUST be a 4-byte aligned, unlabeled landing pad (lpad
0).
|===
