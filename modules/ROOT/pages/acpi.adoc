...
| [[acpi-rimt]]`ACPI_070` | The RISC-V IO Mapping Table cite:[RIMT] MUST be implemented on systems with IOMMU.
|===
...
| [[acpi-trace-dev]]`AML_110` | Systems implementing the RISC-V Efficient Trace (E-trace) protocol or the
RISC-V Nexus-based Trace (N-trace) protocol MUST have RISC-V Trace components in the ACPI namespace as
described in <<acpi-trace, RISC-V ACPI Trace>>.
|===

include::../partials/acpi-id.adoc[leveloffset=-1]
include::../partials/acpi-prop.adoc[leveloffset=-1]
include::../partials/acpi-trace.adoc[leveloffset=-1]
