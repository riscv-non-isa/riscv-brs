...
or an SBI console (SPCR `Interface Type` 0x15). When SPCR describes the SBI console, the operating
system must use the SBI Probe extension (FID #3) to detect the Debug Console Extension (`DBCN`) or
both the Console putchar Extension (EID #1) and the Console getchar Extension (EID #2).
